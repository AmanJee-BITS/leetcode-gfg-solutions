...
public:
    string numberToWords(int num) {
        ...
        vector<string> ten_places = {"",       "",      "Twenty", "Thirty",
        ...
        auto three_digit = [&](int num) {
            ...
            return less_than_twenty[num / 100] + " Hundred" +
                   (num % 100 == 0 ? "" : " " + two_digit(num % 100));
        };
        ...
        if (million)
            res += (res.empty() ? "" : " ") + three_digit(million) + " Million";
        if (thousand)
            res +=
                (res.empty() ? "" : " ") + three_digit(thousand) + " Thousand";
        if (hundred)
            res += (res.empty() ? "" : " ") + three_digit(hundred);
